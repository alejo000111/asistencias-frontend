  ...
  <div class="p-3 bg-light shadow-sm">
    ...
    <ul v-else class="list-group list-group-flush small">
      <li v-for="log in historialFiltrado" :key="log.id" class="list-group-item bg-transparent px-0 d-flex justify-content-between align-items-center border-bottom border-light">
        <div><strong class="text-dark">{{ formatearFecha(log.fecha) }}</strong><br><span class="text-muted" style="font-size: 0.85rem;">{{ log.tipoMovimiento === 'PAGO_DIRECTO' ? ('📦 ' + (log.concepto || 'Pago Directo')) : '💰 Abono' }} ({{ log.metodoPago === 'TRANSFERENCIA' ? 'Transf.' : 'Efectivo' }})</span></div>
...
const historialFiltrado = computed(() => historialPadre.value.filter(log => log.tipoMovimiento === 'INGRESO_ABONO' || log.tipoMovimiento === 'PAGO_DIRECTO'));
...
const eliminarAbono = async (log) => {
  const tipoStr = log.tipoMovimiento === 'PAGO_DIRECTO' ? 'el pago del paquete' : 'el abono';
  if (!confirm(`¿Estás seguro de eliminar ${tipoStr} por $${formatearDinero(log.monto)}?`)) return;
  try {
    ...
    console.error(e);
    const msg = e.response?.data?.mensaje || e.response?.data?.error || 'Error al eliminar el registro.';
    alert('❌ ' + msg);
  }
};
